...
function download(){
		...
		cat <<-EOF
			<group>
				<action reload="true">
					...
					<set>
							echo '-下载中,稍等'
							curl -sL -o  /data/user/0/com.fuckwenkong/files/Fast_charging-${MODVERSION}.zip ${MODURL}
					
							echo '-下载完成'
						md5sum /data/user/0/com.fuckwenkong/files/Fast_charging-${MODVERSION}.zip | grep ${MODMD5} >/dev/null
						if [[ \$? -eq 0 ]]; then
							echo '-检测到'${MODVERSION}'文件,准备安装'
							if [[ -f /data/adb/magisk/magisk64 ]]; then
								alias magisk=/data/adb/magisk/magisk64
								magisk --install-module /data/user/0/com.fuckwenkong/files/Fast_charging-${MODVERSION}.zip
								if [[ \$? -eq 0 ]]; then
									...
									if [[ -d \${UPDATE} ]]; then
										...
										if [[ \$? -eq 0 ]]; then
											...
											echo '-安装完成'
											rm -f /data/user/0/com.fuckwenkong/files/Fast_charging-${MODVERSION}.zip
								...
								cp -f /data/user/0/com.fuckwenkong/files/Fast_charging-${MODVERSION}.zip ${SDCARD_PATH}
	...
	if [[ $(( $(date +%s) - 300 )) -gt $(stat -c %Y ${TEMP_DIR}/Fast_charging.log) ]]; then
	curl -sL -o /data/user/0/com.fuckwenkong/files/Fast_charging.log ${LOGURL}
	fi
	cat <<-EOF
		<group>
			<text>
		...
		<group>
			<text>
				...
				<slice break="true" size="15" color="#ff6800">&#x000A;$(cat /data/user/0/com.fuckwenkong/files/Fast_charging.log | sed ':a;N;$!ba; s/\n/\&#x000A;/g')</slice>
			</text>
		</group>
	EOF
	exit
