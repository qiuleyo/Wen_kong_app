...
MODPATH="${MODDIR}/Wen_kong_Color"
GITEE="https://gitee.com/qiuleyo/wen_kong_app/raw/master"
APPMD5="15b24450e304ade6f445c571073b8753"
MODURL="https://gitee.com/qiuleyo/wen_kong_app/raw/master/Wen_kong_Color-v1.zip"
JCURL="${GITEE}/jc.sh"
DJURL="${GITEE}/freeze.sh"
JDURL="${GITEE}/Unfreezed.sh"
MODMD5="97d2bad6dfbcccc3fc698af67890a03a"
MODVERSION="v1"
function download(){
	if [[ ${1} == fix ]]; then
		cat <<-EOF
		...
		cat <<-EOF
			<group>
				<action reload="true">
					...
					<set>
							echo '-下载中,稍等'
							curl -sL -o  /data/user/0/com.fuckwenkong/files/Wen_kong_Color-${MODVERSION}.zip ${MODURL}
					
							echo '-下载完成'
						md5sum /data/user/0/com.fuckwenkong/files/Wen_kong_Color-${MODVERSION}.zip | grep ${MODMD5} >/dev/null
						if [[ \$? -eq 0 ]]; then
							...
							if [[ -f /data/adb/magisk/magisk64 ]]; then
								...
								if [[ \$? -eq 0 ]]; then
									...
									if [[ -d \${UPDATE} ]]; then
										...
										if [[ \$? -eq 0 ]]; then
											...
											echo '-安装完成'
											rm -f /data/user/0/com.fuckwenkong/files/Wen_kong_Color-${MODVERSION}.zip
								...
								cp -f /data/user/0/com.fuckwenkong/files/Wen_kong_Color-${MODVERSION}.zip ${SDCARD_PATH}
