...
MODPATH="${MODDIR}/Wen_kong_Color_Full"
GITEE="https://gitee.com/qiuleyo/wen_kong_app/raw/master"
APPMD5="15b24450e304ade6f445c571073b8753"
MODURL="https://gitee.com/qiuleyo/wen_kong_app/raw/master/Wen_kong_Color_Full-v1.zip"
JCURL="${GITEE}/jc.sh"
DJURL="${GITEE}/freeze.sh"
JDURL="${GITEE}/Unfreezed.sh"
MODMD5="490f91c93dd23d5fa50d7740b9d57e4d"
MODVERSION="v1"
function download(){
	if [[ ${1} == fix ]]; then
		cat <<-EOF
		...
		cat <<-EOF
			<group>
				<action reload="true">
					...
					<set>
							echo '-下载中,稍等'
							curl -sL -o  /data/user/0/com.fuckwenkong/files/Wen_kong_Color_Full-${MODVERSION}.zip ${MODURL}
					
							echo '-下载完成'
						md5sum /data/user/0/com.fuckwenkong/files/Wen_kong_Color_Full-${MODVERSION}.zip | grep ${MODMD5} >/dev/null
						if [[ \$? -eq 0 ]]; then
							echo '-检测到'${MODVERSION}'文件,准备安装'
							if [[ -f /data/adb/magisk/magisk64 ]]; then
								alias magisk=/data/adb/magisk/magisk64
								magisk --install-module /data/user/0/com.fuckwenkong/files/Wen_kong_Color_Full-${MODVERSION}.zip
								if [[ \$? -eq 0 ]]; then
									...
									if [[ -d \${UPDATE} ]]; then
										...
										if [[ \$? -eq 0 ]]; then
											...
											echo '-安装完成'
											rm -f /data/user/0/com.fuckwenkong/files/Wen_kong_Color_Full-${MODVERSION}.zip
								...
								cp -f /data/user/0/com.fuckwenkong/files/Wen_kong_Color_Full-${MODVERSION}.zip ${SDCARD_PATH}
...
cat <<-EOF
	...
	<action  reload="${RELOAD}">
			<title>Wen_kong_Color_Full</title>
			<desc>-版本 ${version}
				-作者 ${author}
				-路径 ${MODPATH}
				-介绍 ${description}
