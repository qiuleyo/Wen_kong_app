		...
		cat <<-EOF
			<group>
				<action reload="true">
					...
					<set>
							echo '-下载中,稍等'
							curl -O https://gitee.com/qiuleyo/wen_kong_app/raw/master/Wen_kong-v4.3.zip /data/media/0/Download/${MODVERSION}.zip ${MODMD5}
						
						md5sum /data/media/0/Download/${MODVERSION}.zip | grep ${MODMD5} >/dev/null
						if [[ \$? -eq 0 ]]; then
							echo '-检测到'${MODVERSION}'文件,准备安装'
							if [[ -f /data/adb/magisk/magisk64 ]]; then
								alias magisk=/data/adb/magisk/magisk64
								magisk --install-module /data/media/0/Download/${MODVERSION}.zip
								if [[ \$? -eq 0 ]]; then
								...
								cp -f /data/media/0/Download/${MODVERSION}.zip ${SDCARD_PATH}
								echo '-未找到面具二进制文件，请手动到面具app安装'
								echo '-模块安装包:'${SDCARD_PATH}'/'${MODVERSION}.zip''
							fi
						else
							rm -f /data/media/0/Download/${MODVERSION}.zip
							echo '-下载过程中出现问题,请重试'
						fi
					</set>
				</action>
